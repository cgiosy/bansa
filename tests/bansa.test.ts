...
describe("Atom Library - Advanced Tests", () => {
  ...
  it("scope with deep dependencies", async () => {
    ...
    const scope1 = createScope(identity, [[$x1, 11]]);
    const scope2 = createScope(identity, [[$x2, 22]]);
  ...
  it("deep scope", async () => {
    ...
    const scope2 = createScope(scope1, [[$x2, 22]]);
  ...
  it("deep scope (self referenced)", async () => {
    const $x = $(1);
    const identity = (x: any) => x;
    const scope1 = createScope(identity, [[$x, $((get) => get($x) + 1)]]);
    const scope2 = createScope(scope1, [[$x, $((get) => get($x) + 1)]]);
...
describe("Bansa Documentation Examples as Tests", () => {
  ...
  describe("Atom Granularity Example", () => {
    ...
    it("Bad Practice: Combined state causes unnecessary re-fetches", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith("/users/123", expect.anything());
      expect(mockFetch).toHaveBeenCalledWith("/posts/456", expect.anything());
      ...
      expect(mockFetch).toHaveBeenCalledTimes(2);
      expect(mockFetch).toHaveBeenCalledWith("/users/123", expect.anything()); // 불필요한 호출
      expect(mockFetch).toHaveBeenCalledWith("/posts/789", expect.anything());
    ...
    it("Good Practice: Split states prevent unnecessary re-fetches", async () => {
      ...
      const $user = $((get, { signal }) =>
        fetch(`/users/${get($userId)}`, { signal }).then((res) => res.json()),
      );

      const $postId = $(456);
      const $post = $((get, { signal }) =>
        fetch(`/posts/${get($postId)}`, { signal }).then((res) => res.json()),
      );
      ...
      const pageDataSub = $pageData.subscribe(({ userName, postAuthor }) => {
      ...
      expect(userElm.innerHTML).toBe("John Doe");
      expect(postElm.innerHTML).toBe("<p>Post content</p>");
      expect(commentElm.innerHTML).toBe("Hello John Doe! Comment to Jane Smith.");
      ...
      expect(mockFetch).toHaveBeenCalledWith("/posts/789", expect.anything());
      expect(mockFetch).not.toHaveBeenCalledWith("/users/123", expect.anything());
      expect(commentElm.innerHTML).toBe("Hello John Doe! Comment to Jane Smith."); // post.author가 업데이트됨
    ...
    it("Pattern 1: Atom returning an atom for shared resources", async () => {
      ...
      const $wsConnection = $((_, { signal }) => {
        ...
        return {
          ...
          addEventListener: (listener: (data: any) => void, listenerSignal: ThenableSignal) => {
            mockWs.addEventListener("message", listener);
            listenerSignal.then(() => mockWs.removeEventListener("message", listener));
          },
        };
      });
  ...
  describe("Debounce-Throttling Example", () => {
    ...
    const delayedState = <Value>(initial: Value, minDelay: number, maxDelay: number) => {
      const $value = $(initial);
  ...
  describe("Scope", () => {
    ...
    it("scope with updates (2)", async () => {
      const $x = $(0);
      const $y = $((get) => get($x) + 1);
      const scope = createScope(null, [[$x, 100]]);
      const $x2 = scope($x);
      const $y2 = scope($y);
    ...
    it("scope with updates (3)", async () => {
      ...
      const $y = $((get) => get($x) + 1);
      const $x2 = $(100);
      const scope = createScope(null, [[$x, $x2]]);
      const $y2 = scope($y);
