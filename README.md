...
`$`의 두 번째 파라미터로 옵션을 전달할 수 있습니다. 옵션 객체에서 `global`가 `true`로 설정되어 있다면, 해당 객체는 한 번 활성화되면 다시 비활성화되지 않습니다. 추가로 스코프에서도 새로 만들어지지 않는데, 자세한 설명은 생략합니다. 활성 상태를 유지하려고 무의미한 구독을 추가하는 대신 사용할 수 있습니다.
